...
Description:
This is a DL algorithm using CNN for HAR
...
def refine_classification(pima):
    ...
    while (onTrainIndex > 0):
        onTrainIndex, stationId = clasifyOnTrain(pima, onTrainIndex, stationId)
...
class HAR_CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)  # change order
...
def main():
    ...
    # Model Setup using CNN functions 
    input_dim = X.shape[1]
    num_classes = len(label_classes)
    model = HAR_CNN(input_dim=input_dim, num_classes=num_classes)
    model.to(device)
